...
ofstream rwLogFile, avgTimeFile;
vector<long long> worstCaseReaderRW, worstCaseWriterRW;
...
int main() {
    ...
    rwLogFile.open("RW-log.txt");
    avgTimeFile.open("AverageTimeRW.txt");
    ...
    vector<long long> entryTimesRW, entryTimesWriterRW;

    // Create writer threads for RW algorithm
    thread writerThreadsRW[nw];
    for (int i = 0; i < nw; ++i) {
        writerThreadsRW[i] = thread(writer, i + 1, kw, muCS, muRem, ref(entryTimesWriterRW), ref(rwLogFile));
    }

    // Create reader threads for RW algorithm
    thread readerThreadsRW[nr];
    for (int i = 0; i < nr; ++i) {
        readerThreadsRW[i] = thread(reader, i + 1, kr, muCS, muRem, ref(entryTimesRW), ref(rwLogFile));
    }

    // Join writer threads for RW algorithm
    for (int i = 0; i < nw; ++i) {
        writerThreadsRW[i].join();
    }

    // Join reader threads for RW algorithm
    for (int i = 0; i < nr; ++i) {
        readerThreadsRW[i].join();
    }

    // Calculate and write average time for RW algorithm reader and writer threads
    writeAverageTimeToFile(avgTimeFile, entryTimesRW, "RW Reader");
    writeAverageTimeToFile(avgTimeFile, entryTimesWriterRW, "RW Writer");

    // Write worst-case times to file
    writeWorstCaseTimesToFile(avgTimeFile, worstCaseReaderRW, "RW Reader");
    writeWorstCaseTimesToFile(avgTimeFile, worstCaseWriterRW, "RW Writer");
...
void reader(int id, int kr, int muCS, int muRem, vector<long long>& entryTimes, ofstream& logFile) {
    ...
    worstCaseReaderRW.push_back(worstCase); // Store worst-case waiting time
...
void writer(int id, int kw, int muCS, int muRem, vector<long long>& entryTimes, ofstream& logFile) {
    ...
    sem_wait(&mut); // Acquire mut semaphore before modifying the shared vector
    worstCaseWriterRW.push_back(worstCase); // Store worst-case waiting time
    sem_post(&mut); // Release mut semaphore after modifying the shared vector
}
